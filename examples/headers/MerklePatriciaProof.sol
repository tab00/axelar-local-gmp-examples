...
library MerklePatriciaProof {
   ...
   */
  function get(bytes memory encodedPath, bytes[] memory rlpParentNodes, bytes32 root) internal pure returns (bytes memory) {
    ...
    if (path.length == 0) {revert('1');}

    for (uint i = 0; i < rlpParentNodes.length; i++) {
      if (pathPtr > path.length) {revert('2');}
      currentNode = rlpParentNodes[i];
      RLPReader.RLPItem memory parentNode = RLPReader.toRlpItem(currentNode);
      if (nodeKey != keccak256(currentNode)) {revert('3');}
      currentNodeList = RLPReader.toList(parentNode);
